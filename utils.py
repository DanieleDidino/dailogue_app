...
def return_category_sentence(clf_text: str) -> tuple[list[str], list[str]]:
    """
    Return a list with the categories and a list with the corresponding sentences.

    Args:
        clf_text (string): Text outputed by OpenAI API via get_responses_from_llm()

    Returns:
        categories: A list of categories extracted from clf_text
        sentences: A list of sentences extracted from clf_text
    """
    
    # Pattern to find the sentences
    pattern_sentence = r"Text: ([^\n]+)"
    # extract the sentences
    sentences = re.findall(pattern_sentence, clf_text)

    # Pattern to find the categories
    pattern_category = r"Category: ([^\n]+)"
    # extract the categories
    categories = re.findall(pattern_category, clf_text)
